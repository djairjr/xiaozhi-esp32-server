...
class ManageApiClient:
    ...
    def _init_client(cls, config):
        ...
        if not cls.config.get("url") or not cls.config.get("secret"):
            raise Exception("The url or secret configuration of manager-api is wrong")

        if "you" in cls.config.get("secret"):
            raise Exception("Please configure the secret of manager-api first")
        ...
        # NOTE(goody): 2025/4/16 http-related resources are managed in a unified manner, and thread pools or timeouts can be added later.
        # In the future, you can also uniformly configure apiToken and other general Auth.
        cls._client = httpx.Client(
            ...
            timeout=cls.config.get("timeout", 30),  # Default timeout is 30 seconds
        )

    @classmethod
    def _request(cls, method: str, endpoint: str, **kwargs) -> Dict:
        """Send a single HTTP request and process the response"""
        ...
        if result.get("code") == 10041:
            ...
            raise Exception(f"API returns error: {result.get('msg', 'Unknown error')}")
    ...
    def _execute_request(cls, method: str, endpoint: str, **kwargs) -> Dict:
        ...
        while retry_count <= cls.max_retries:
            try:
                ...
                # Determine whether you should try again
                if retry_count < cls.max_retries and cls._should_retry(e):
                    retry_count += 1
                    print(
                        f"{method} {endpoint} request failed, will be retried {retry_count} times after {cls.retry_delay:.1f} seconds"
                    )
                    time.sleep(cls.retry_delay)
                    continue
                else:
                    # Without retrying, an exception is thrown directly.
                    raise
...
def get_server_config() -> Optional[Dict]:
    """Get basic server configuration"""
...
def report(
    ...
    """Business method example with circuit breakers"""
    ...
    try:
        ...
        print(f"TTS reporting failure: {e}")
        return None
