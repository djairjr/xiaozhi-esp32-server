...
def check_config_file():
    ...
    if config_file_valid:
        return
    """Simplified configuration checks that only prompt users for profile usage"""
    custom_config_file = get_project_dir() + "data/." + default_config_file
    if not os.path.exists(custom_config_file):
        raise FileNotFoundError(
            "The data/.config.yaml file cannot be found. Please follow the tutorial to confirm whether the configuration file exists."
        )
    ...
    if config.get("read_config_from_api", False):
        ...
        if old_config_origin.get("selected_module") is not None:
            error_msg = "Your configuration file seems to contain both the smart console configuration and the local configuration:\n"
            error_msg += "\nRecommend you:\n"
