...
3. Use ConnectionHandler.enqueue_tts_report method to report
...
def opus_to_wav(conn, opus_data):
    """Convert Opus data to byte stream in WAV format

    Args:
        output_dir: output directory (parameters retained to maintain interface compatibility)
        opus_data: opus audio data
...
def enqueue_tts_report(conn, text, opus_data):
    ...
    Args:
        ...
        opus_data: opus audio data"""
    try:
        # Use a queue of connection objects, passing in text and binary data instead of file paths
        if conn.chat_history_conf == 2:
            ...
            conn.logger.bind(tag=TAG).debug(
                f"TTS data has been added to the reporting queue: {conn.device_id}, audio is not reported"
            )
    except Exception as e:
...
def enqueue_asr_report(conn, text, opus_data):
    ...
    try:
        ...
        if conn.chat_history_conf == 2:
            ...
            conn.logger.bind(tag=TAG).debug(
                f"ASR data has been added to the reporting queue: {conn.device_id}, audio is not reported"
            )
    except Exception as e:
