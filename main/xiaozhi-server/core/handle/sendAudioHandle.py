...
async def sendAudio(conn, audios, frame_duration=60):
    ...
    if isinstance(audios, bytes):
        # Reset the flow control state when reading for the first time and when the session changes
        ...
        # Pre-buffering: the first 5 packets are sent directly without delay
        pre_buffer_count = 5
        flow_control = conn.audio_flow_control
        current_time = time.perf_counter()

        if flow_control["packet_count"] < pre_buffer_count:
            # Pre-buffering stage, direct sending without delay
            pass
        elif send_delay > 0:
            # Use fixed delay
    ...
    try:
        ...
        if text.strip().startswith("{") and text.strip().endswith("}"):
            ...
            if isinstance(parsed_data, dict) and "content" in parsed_data:
                # If it is in JSON format containing speaker information, only the content part is displayed.
