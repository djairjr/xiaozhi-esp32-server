...
async def handle_user_intent(conn, text):
    # Preprocess input text, handling possible JSON formats
    try:
    ...
    if conn.intent_type == "function_call":
        # Use chat methods that support function calling, no more intent analysis
        return False
    # Intent analysis using LLM
    intent_result = await analyze_intent_with_llm(conn, text)
    if not intent_result:
        return False
    # Sentence_id is generated when the session starts
    conn.sentence_id = str(uuid.uuid4().hex)
    # Handle various intentions
...
async def check_direct_exit(conn, text):
    ...
    for cmd in cmd_exit:
        if text == cmd:
            conn.logger.bind(tag=TAG).info(f"Explicit exit command recognized: {text}")
...
async def process_intent_result(conn, intent_result, original_text):
    """Processing intent recognition results"""
    try:
        # Try parsing the result into JSON
        intent_data = json.loads(intent_result)

        # Check if there is function_call
        if "function_call" in intent_data:
            # function_call is obtained directly from intent recognition
            ...
            if function_name == "result_for_context":
                ...
                def process_context_result():
                    ...
                    # Build contextual foundational prompts
            ...
            if "arguments" in intent_data["function_call"]:
                ...
                if function_args is None:
                    function_args = {}
            # Make sure the parameter is JSON in string format
            ...
            # Use executors to perform function calls and result processing
