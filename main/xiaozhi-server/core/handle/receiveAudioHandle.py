...
async def handleAudioMessage(conn, audio):
    ...
    # If the device has just woken up, briefly ignore VAD detection
    if hasattr(conn, "just_woken_up") and conn.just_woken_up:
        ...
        return
    # In manual mode, the content being played will not be interrupted.
    if have_voice:
        if conn.client_is_speaking and conn.client_listen_mode != "manual":
            await handleAbortMessage(conn)
    # Device idle detection for a long time, used to say goodbye
...
async def resume_vad_detection(conn):
    # Wait 2 seconds and then resume VAD detection.
...
async def startToChat(conn, text):
    ...
    # Intent is not handled, continue the regular chat flow with actual text content
