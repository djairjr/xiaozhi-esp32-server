...
class ConnectionHandler:
    def __init__(
        ...
        # In the future, you can adjust the reporting of asr and tts by modifying this. Currently, both are enabled by default.
        ...
        # Manage voiceprint recognition individually for each connection
        ...
        self.last_activity_time = 0.0  # Uniform activity timestamp (milliseconds)
        self.client_voice_stop = False
        self.last_is_voice = False

        # asr related variables
        # Because the public local ASR may be used during actual deployment, variables cannot be exposed to the public ASR.
        # Therefore, variables related to ASR need to be defined here, which are private variables of connection.
        self.asr_audio = []
        ...
        )  # Add 60 seconds to the original first closure to perform the second closure
        self.timeout_task = None

        # {"mcp":true} means enabling the MCP function
        self.features = None

        # Marks whether the connection is from MQTT
        self.conn_from_mqtt_gateway = False

        # Initialize prompt word manager
    ...
    async def handle_connection(self, ws):
        try:
            ...
            # Check if coming from MQTT connection
            ...
            try:
                ...
                self.logger.bind(tag=TAG).info("Client disconnects")
            ...
            try:
                ...
                # Make sure to close the connection even if saving memory fails
                try:
                    await self.close(ws)
    ...
    async def _save_and_close(self, ws):
        ...
        try:
            if self.memory:
                # Save memory asynchronously using a thread pool
    ...
    async def _route_message(self, message):
        ...
        if isinstance(message, str):
            ...
            # Handle audio packets from MQTT gateway
            if self.conn_from_mqtt_gateway and len(message) >= 16:
                handled = await self._process_mqtt_audio_message(message)
                if handled:
                    return

            # When header processing is not required or there is no header, the original message is processed directly.
            self.asr_audio_queue.put(message)

    async def _process_mqtt_audio_message(self, message):
        """Process audio messages from the MQTT gateway, parse the 16-byte header and extract the audio data

        Args:
            message: audio message including header
        ...
        try:
            ...
            if audio_length > 0 and len(message) >= 16 + audio_length:
                # Extract accurate audio data with specified length
                audio_data = message[16 : 16 + audio_length]
                # Sorting based on timestamp
                self._process_websocket_audio(audio_data, timestamp)
                return True
            elif len(message) > 16:
                # The length is not specified or the length is invalid. Remove the header and process the remaining data.
        ...
        # If the processing fails, False is returned to indicate that processing needs to continue.
        return False

    def _process_websocket_audio(self, audio_data, timestamp):
        """Handling audio packets in WebSocket format"""
    ...
    async def handle_restart(self, message):
        """Handle server restart requests"""
        try:
            ...
            def restart_server():
                """How to actually perform a restart"""
                time.sleep(1)
                self.logger.bind(tag=TAG).info("Performing a server restart...")
                subprocess.Popen(
            ...
            # Use thread execution restart to avoid blocking the event loop
    ...
    def _initialize_components(self):
        try:
            ...
            if self.config.get("prompt") is not None:
                ...
                self.logger.bind(tag=TAG).info(
                    f"Quick initialization component: prompt successful {prompt[:50]}..."
                )
    ...
    def _initialize_asr(self):
        ...
        if self._asr.interface_type == InterfaceType.LOCAL:
            ...
            # Because a local instance ASR can be shared by multiple connections
            asr = self._asr
        else:
            # If the public ASR is a remote service, initialize a new instance
            # Because remote ASR involves websocket connections and receiving threads, one instance is required for each connection.
        ...
        try:
            ...
            if voiceprint_config:
                ...
                if voiceprint_provider is not None and voiceprint_provider.enabled:
                    ...
                    self.logger.bind(tag=TAG).info("Voiceprint recognition has been dynamically enabled upon connection")
                else:
                    self.logger.bind(tag=TAG).warning("The voiceprint recognition function is enabled but the configuration is incomplete.")
            else:
    ...
    def _initialize_private_config(self):
        """If it is obtained from the configuration file, it will be instantiated twice."""
        if not self.read_config_from_api:
            return
        """Obtain differentiated configuration from the interface for secondary instantiation, non-full re-instantiation"""
        try:
            ...
            self.logger.bind(tag=TAG).info(
                f"{time.time() - begin_time} seconds, successfully obtain differentiated configuration: {json.dumps(filter_sensitive_info(private_config), ensure_ascii=False)}"
            )
            ...
            self.need_bind = True
            self.logger.bind(tag=TAG).error(f"Failed to obtain differentiated configuration: {e}")
            private_config = {}
    ...
    def _initialize_memory(self):
        ...
        # Use mem_local_short mode
            ...
            if memory_llm_name and memory_llm_name in self.config["LLM"]:
                # If dedicated LLM is configured, create a standalone LLM instance
                ...
                self.logger.bind(tag=TAG).info("Using the main LLM as an intent recognition model")
    ...
    def _initialize_intent(self):
        ...
        """Load Unified Tools Processor"""
        self.func_handler = UnifiedToolHandler(self)

        # Asynchronous initialization tool handler
    ...
    def chat(self, query, depth=0):
        ...
        # Create a new session ID and send a FIRST request for the top level
        if depth == 0:
        ...
        if tool_call_flag:
            ...
            if not bHasError:
                # If a large model needs to be processed first, add relevant post-processing log information.
        ...
        self.llm_finish_task = True
        # Using lambda lazy calculation, get_llm_dialogue() is only executed at DEBUG level
        self.logger.bind(tag=TAG).debug(
            lambda: json.dumps(
    ...
    def _report_worker(self):
        ...
        while not self.stop_event.is_set():
            try:
                # Get data from queue, set timeout to periodically check for stop events
                item = self.report_queue.get(timeout=1)
                if item is None:  # Detect poison pill objects
                    break
                try:
        ...
        self.logger.bind(tag=TAG).info("The chat record reporting thread has exited")

    def _process_report(self, type, text, audio_data, report_time):
        """Handle reported tasks"""
        try:
            # Execute reporting (incoming binary data)
    ...
    def clearSpeakStatus(self):
        self.client_is_speaking = False
        self.logger.bind(tag=TAG).debug(f"Clear server talk status")
            ...
            try:
                if ws:
                    ...
                    try:
                        ...
                        # If shutdown fails, ignore errors
                        pass
                elif self.websocket:
                    try:
                        if (
    ...
    async def _check_timeout(self):
        ...
        try:
            while not self.stop_event.is_set():
                # Check for timeout (only if timestamp has been initialized)
                if self.last_activity_time > 0.0:
                    ...
                    if (
                        ...
                        if not self.stop_event.is_set():
                            ...
                            # Set stop events to prevent repeated processing
                            self.stop_event.set()
                            # Use try-except to wrap the shutdown operation to ensure it does not block due to exceptions
                            try:
                                await self.close(self.websocket)
                            except Exception as close_error:
                                self.logger.bind(tag=TAG).error(
                                    f"Timeout error in closing connection: {close_error}"
                                )
                        break
