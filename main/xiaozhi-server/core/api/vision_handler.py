...
class VisionHandler:
    ...
    async def handle_post(self, request):
        """Handling MCP Vision POST requests"""
        ...
        try:
            ...
            if not vllm_type:
                raise ValueError(f"Unable to find provider {vllm_type} for VLLM module")
    ...
    async def handle_get(self, request):
        ...
        try:
            ...
            if vision_explain and len(vision_explain) > 0 and "null" != vision_explain:
                message = (
                    f"The MCP Vision interface is running normally, and the address of the visual explanation interface is: {vision_explain}"
                )
            else:
