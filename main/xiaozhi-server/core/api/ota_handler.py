...
class OTAHandler(BaseHandler):
    ...
    def generate_password_signature(self, content: str, secret_key: str) -> str:
        ...
        try:
            ...
            self.logger.bind(tag=TAG).error(f"Failed to generate MQTT cryptographic signature: {e}")
            return ""
        ...
        try:
            ...
            if mqtt_gateway_endpoint:  # If a non-empty string is configured
                ...
                if signature_key:
                    ...
                    self.logger.bind(tag=TAG).warning("MQTT signing key is missing, password left blank")
                ...
                self.logger.bind(tag=TAG).info(f"Deliver MQTT gateway configuration to device {device_id}")
