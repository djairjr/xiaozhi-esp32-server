...
class LLMProvider(LLMProviderBase):
    def __init__(self, config):
        ...
        self.session_conversation_map = {}  # Stores the mapping between session_id and conversation_id
                ...
                if self.mode == "chat-messages":
                    for line in r.iter_lines():
                        if line.startswith(b"data: "):
                            ...
                            # Filter message_replace event, this event will be pushed once
    ...
    def response_with_functions(self, session_id, dialogue, functions=None):
        if len(dialogue) == 2 and functions is not None and len(functions) > 0:
            # The first time you call llm, get the last user message and append the tool prompt word
        ...
        # If the last one is role="tool", it is appended to user
