...
class TTSProvider(TTSProviderBase):
    def __init__(self, config, delete_audio_file):
        ...
        if isinstance(self.params, str):
            try:
                ...
                raise ValueError("Custom TTS configuration parameter error, unable to parse string into object")
        elif not isinstance(self.params, dict):
            raise TypeError("Custom TTS configuration parameter error, please refer to the configuration instructions")
    ...
    async def text_to_speak(self, text, output_file):
        ...
        if resp.status_code == 200:
            ...
            raise Exception(error_msg)  # Throw an exception and let the caller catch it
