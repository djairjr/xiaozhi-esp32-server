...
class TTSProvider(TTSProviderBase):
    ...
    async def pcm_to_wav(self, pcm_data: bytes, sample_rate: int = 24000, num_channels: int = 1,
        ...
        :return: Byte data in WAV format"""
    ...
    async def text_streaming(self, text, output_file):
        try:
            # Connect to a WebSocket server asynchronously using websockets
            async with websockets.connect(self.url) as ws:
                ...
                try:
                    while True:
                        ...
                        if status == 2:  # last packet
                            ...
                            # Splicing audio data (base64 encoded PCM data)
                            audio_chunks += base64.b64decode(response.get("audio"))
                except asyncio.TimeoutError:
                    raise Exception(f"WebSocket timeout: Waiting for audio data for more than {timeout_seconds} seconds")

                # Convert spliced ​​PCM data to WAV format
                ...
                end_request = {
                    "task": "tts",
                    "signal": "end",
                    "session": session_id  # Session ID must match the one in the start request
                }
                ...
                # Determine whether to save the file based on configuration
