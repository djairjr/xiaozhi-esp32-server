...
class TTSProvider(TTSProviderBase):
    def __init__(self, config, delete_audio_file):
        ...
        if not all([self.app_id, self.api_key, self.api_secret]):
            raise ValueError("Iflytek TTS needs to configure app_id, api_key and api_secret")
                ...
                if ContentType.TEXT == message.content_type:
                    if message.content_detail:
                        try:
                            ...
                            # Do not use continue to ensure that subsequent processing is not interrupted
            ...
            if self.ws:
                ...
                if self._monitor_task:
                    try:
                        ...
                        logger.bind(tag=TAG).error(f"An error occurred while waiting for the listening task to complete: {str(e)}")
                    finally:
                        self._monitor_task = None
        except Exception as e:
                        ...
                        if code == 0:
                            ...
                            if audio_payload:
                                ...
                                if status == 0:
                                    ...
                                    logger.bind(tag=TAG).debug("After receiving the audio data in the end state, TTS synthesis is completed.")
    ...
    def _build_base_request(self, status,text=" "):
        """Build the basic request structure"""
        return {
            "header": {
                "app_id": self.app_id,
                "status": status,
