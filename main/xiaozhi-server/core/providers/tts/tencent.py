...
class TTSProvider(TTSProviderBase):
    ...
    def _get_auth_headers(self, request_body):
        ...
        # The request header must contain host and content-type, and are arranged in dictionary order
        canonical_headers = (
    ...
    async def text_to_speak(self, text, output_file):
        ...
        request_json = {
            ...
            "VoiceType": int(self.voice),  # timbre
        }

        try:
            # Get request headers (regenerated on every request to ensure timestamps and signatures are up to date)
            ...
            if resp.status_code == 200:
                ...
                if response_data.get("Response", {}).get("Error") is not None:
                    ...
                    raise Exception(
                        f"API returns error: {error_info['Code']}: {error_info['Message']}"
                    )
