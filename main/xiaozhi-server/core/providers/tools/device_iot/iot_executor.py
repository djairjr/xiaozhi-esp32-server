...
class DeviceIoTExecutor(ToolExecutor):
    ...
    async def execute(
        ...
        try:
            # Parse the tool name and obtain the device name and operation type
            if tool_name.startswith("get_"):
                ...
                if len(parts) >= 2:
                    ...
                    # Handling placeholders in responses
            ...
            return ActionResponse(action=Action.ERROR, response="Unable to resolve IoT tool name")
                        ...
                        "function": {
                            ...
                            "parameters": {
                                ...
                                "properties": {
                                    "response_success": {
                                        "type": "string",
                                        "description": f"A friendly reply when the query is successful. {{value}} must be used as a placeholder to represent the query value.",
                                    },
                                    "response_failure": {
                                        "type": "string",
    ...
    def get_tools(self) -> Dict[str, ToolDefinition]:
        """Get all on-device IoT tools"""
        return self.iot_tools.copy()

    def has_tool(self, tool_name: str) -> bool:
        """Check if there is a specified device-side IoT tool"""
