"""IoT device support module, providing IoT device descriptors and status processing"""
...
async def handleIotDescriptors(conn, descriptors):
    ...
    while (
        ...
        wait_max_time -= 1
        if wait_max_time <= 0:
            logger.bind(tag=TAG).debug("The connection object does not have func_handler")
            return

    functions_changed = False

    for descriptor in descriptors:
        # If the descriptor does not have properties and methods, skip it directly.
                ...
                for method_name, method_info in descriptor["methods"].items():
                    if "parameters" in method_info:
                        for param_name, param_info in method_info["parameters"].items():
                            # Convert parameter information into attribute information
    ...
    # If a new function is registered, update the function description list
    if functions_changed and hasattr(conn, "func_handler"):
        # Register IoT tools to the unified tool processor
