...
class ToolManager:
    """Unified tool manager to manage all types of tools"""
    ...
    def register_executor(self, tool_type: ToolType, executor: ToolExecutor):
        ...
        self.logger.info(f"Register tool executor: {tool_type.value}")
    ...
    def get_all_tools(self) -> Dict[str, ToolDefinition]:
        ...
        for tool_type, executor in self.executors.items():
            try:
                ...
                self.logger.error(f"Error getting {tool_type.value} tool: {e}")
    ...
    async def execute_tool(
        ...
        try:
            ...
            # Execution tool
            self.logger.info(f"Execution tool: {tool_name}, parameters: {arguments}")
    ...
    def get_tool_statistics(self) -> Dict[str, int]:
        ...
        for tool_type, executor in self.executors.items():
            try:
                ...
                self.logger.error(f"Error getting {tool_type.value} tool statistics: {e}")
