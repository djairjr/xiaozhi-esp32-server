...
async def send_mcp_message(conn, payload: dict):
    ...
    if not conn.features.get("mcp"):
        logger.bind(tag=TAG).warning("The client does not support MCP and cannot send MCP messages.")
        return
...
async def handle_mcp_message(conn, mcp_client: MCPClient, payload: dict):
    """Process MCP messages, including initialization, tool list and tool call response, etc."""
    logger.bind(tag=TAG).info(f"Processing MCP messages: {str(payload)[:100]}")

    if not isinstance(payload, dict):
        logger.bind(tag=TAG).error("MCP message is missing payload field or has wrong format")
        return
    ...
    if "result" in payload:
        ...
        if msg_id == 1:  # mcpInitializeID
            logger.bind(tag=TAG).debug("MCP initialization response received")
            ...
            logger.bind(tag=TAG).debug("MCP tool list response received")
            if isinstance(result, dict) and "tools" in result:
                ...
                logger.bind(tag=TAG).info(
                    f"Number of tools supported by the client device: {len(tools_data)}"
                )
                ...
                for i, tool in enumerate(tools_data):
                    ...
                    logger.bind(tag=TAG).debug(f"Client Tools #{i+1}: {name}")
                ...
                if next_cursor:
                    logger.bind(tag=TAG).info(f"There are more tools, nextCursor: {next_cursor}")
                    await send_mcp_tools_list_continue_request(conn, next_cursor)
                else:
                    await mcp_client.set_ready(True)
                    logger.bind(tag=TAG).info("All tools acquired, MCP client ready")

                    # Refresh the tool cache to ensure that the MCP tool is included in the function list
...
async def send_mcp_initialize_message(conn):
    ...
    logger.bind(tag=TAG).info("Send MCP initialization message")
...
async def call_mcp_tool(
    ...
    """Call the specified tool and wait for the response"""
    ...
    try:
        ...
        logger.bind(tag=TAG).info(
            f"The client mcp tool calls {actual_name} successfully, the original result: {raw_result}"
        )
