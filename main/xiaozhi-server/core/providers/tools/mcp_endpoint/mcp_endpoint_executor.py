"""MCP Access Point Tool Executor"""
...
class MCPEndpointExecutor(ToolExecutor):
    ...
    async def execute(
        ...
        try:
            ...
            # Call the MCP access point tool
            ...
            # The large visual model does not undergo secondary LLM processing.
            if (
    ...
    def get_tools(self) -> Dict[str, ToolDefinition]:
        """Get all MCP access point tools"""
        if (
    ...
    def has_tool(self, tool_name: str) -> bool:
        """Check if there is a specified MCP access point tool"""
        if (
            not hasattr(self.conn, "mcp_endpoint_client")
            or not self.conn.mcp_endpoint_client
        ):
