...
class ASRProviderBase(ABC):
        ...
        try:
            ...
            def run_asr():
                ...
                try:
                    ...
                    try:
                        ...
                        logger.bind(tag=TAG).info(f"ASR takes: {end_time - start_time:.3f}s")
                        return result
                    finally:
                        loop.close()
                except Exception as e:
            ...
            def run_voiceprint():
                ...
                try:
                    ...
                    asyncio.set_event_loop(loop)
                    try:
                        # Using a connected voiceprint recognition provider
                        result = loop.run_until_complete(
            ...
            # Run in parallel using thread pool executors
            ...
            if speaker_name:
                logger.bind(tag=TAG).info(f"Identify the speaker: {speaker_name}")
    ...
    def _build_enhanced_text(self, text: str, speaker_name: Optional[str]) -> str:
        """Construct text containing speaker information"""
    ...
    def _pcm_to_wav(self, pcm_data: bytes) -> bytes:
        ...
        if len(pcm_data) == 0:
            logger.bind(tag=TAG).warning("PCM data is empty and WAV cannot be converted")
            return b""
        
        # Make sure the data length is even (16 bit audio)
        if len(pcm_data) % 2 != 0:
            pcm_data = pcm_data[:-1]
    ...
    def decode_opus(opus_data: List[bytes]) -> List[bytes]:
        ...
        try:
            ...
            pcm_data = []
            buffer_size = 960  # Processing 960 samples at a time (60ms at 16kHz)
