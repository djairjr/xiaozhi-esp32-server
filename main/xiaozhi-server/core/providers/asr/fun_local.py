...
class CaptureOutput:
    ...
    def __exit__(self, exc_type, exc_value, traceback):
        ...
        # Output the captured content through logger
        if self.output:
...
class ASRProvider(ASRProviderBase):
    def __init__(self, config: dict, delete_audio_file: bool):
        ...
        # Memory detection, requires more than 2G
        min_mem_bytes = 2 * 1024 * 1024 * 1024
        total_mem = psutil.virtual_memory().total
        if total_mem < min_mem_bytes:
            logger.bind(tag=TAG).error(f"The available memory is less than 2G, currently only {total_mem / (1024*1024):.2f} MB. FunASR may not be started.")
        
        self.interface_type = InterfaceType.LOCAL
        self.model_dir = config.get("model_dir")
        self.output_dir = config.get("output_dir")  # Correct configuration key name
        ...
        while retry_count < MAX_RETRIES:
            try:
                ...
                if not self.delete_audio_file:
                    ...
                    if free_space < len(combined_pcm_data) * 2:  # Reserve 2 times the space
