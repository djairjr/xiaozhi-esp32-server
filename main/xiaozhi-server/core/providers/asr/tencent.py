...
class ASRProvider(ASRProviderBase):
    ...
    async def speech_to_text(
        ...
        try:
            ...
            if not self.secret_id or not self.secret_key:
                logger.bind(tag=TAG).error("Tencent Cloud speech recognition configuration is not set and recognition cannot be performed")
    ...
    def _build_request_body(self, base64_audio: str) -> str:
        ...
        request_map = {
            ...
            "EngSerViceType": "16k_zh",  # Common Chinese Mandarin
            "SourceType": 1,  # The audio data source is a voice file
    ...
    def _get_auth_headers(self, request_body: str) -> Tuple[str, str]:
        ...
        try:
            ...
            # Splicing voucher range
            credential_scope = f"{date}/{service}/tc3_request"

            # Using TC3-HMAC-SHA256 signing method
            ...
            # Note: The header information needs to be arranged in ascending ASCII order, and the key and value must be converted to lowercase.
            # Must contain content-type and host headers
            content_type = "application/json; charset=utf-8"
            host = "asr.tencentcloudapi.com"
            action = "SentenceRecognition"  # Interface name

            # Construct standard header information, paying attention to order and format
            canonical_headers = (
            ...
            # Calculate the hash value of the canonical request
            ...
            logger.bind(tag=TAG).error(f"Failed to generate authentication header: {e}", exc_info=True)
            raise RuntimeError(f"Failed to generate authentication header: {e}")
    ...
    def _send_request(
        ...
        try:
            ...
            if "Response" in response_json and "Error" in response_json["Response"]:
                ...
                raise IOError(f"API returns error: {error_code}: {error_message}")
