...
class ASRProvider(ASRProviderBase):
    def __init__(self, config, delete_audio_file):
        ...
        if self.access_key_id and self.access_key_secret:
            ...
            raise ValueError("Access_key_id+access_key_secret or token must be provided directly")
        ...
        try:
            while self.asr_ws and not conn.stop_event.is_set():
                try:
                    ...
                    # Receiving TranscriptionStarted indicates that the server is ready to receive audio data
    ...
    async def _cleanup(self, conn):
        ...
        if self.asr_ws and should_stop:
            try:
                ...
                logger.bind(tag=TAG).error(f"ASR termination request failed: {e}")
