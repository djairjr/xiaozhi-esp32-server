    ...
    async def speech_to_text(self, opus_data: List[bytes], session_id: str, audio_format="opus") -> Tuple[Optional[str], Optional[str]]:
        ...
        try:
            ...
            logger.bind(tag=TAG).debug(
                f"Audio file saving time: {time.time() - start_time:.3f}s | Path: {file_path}"
            )
            ...
            # Pass the model name using the data parameter
            data = {
                "model": self.model
            }


            with open(file_path, "rb") as audio_file:  # Use the with statement to ensure the file is closed
                files = {
                    "file": audio_file
                }
