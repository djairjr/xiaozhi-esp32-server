        ...
        while True:
            try:
                ...
                if time.time() - self.start_time > self.timeout:
                    raise TimeoutError("Timeout getting file lock")
                time.sleep(0.1)
...
class WakeupWordsConfig:
    ...
    def _load_config(self) -> Dict:
        """Load the configuration file and use the caching mechanism"""
        current_time = time.time()

        # If the cache is valid, return to the cache directly
        if (
    ...
    def _save_config(self, config: Dict):
        """Save configuration to file, protected with file lock"""
        try:
            ...
            print(f"An unknown error occurred while saving the configuration file: {e}")
            raise
    ...
    def update_wakeup_response(self, voice: str, file_path: str, text: str):
        """Update wake word reply configuration"""
        try:
            # Filter emojis
            ...
            print(f"Failed to update wake word reply configuration: {e}")
            raise

    def generate_file_path(self, voice: str) -> str:
        """Generate the audio file path, using the hash value of voice as the file name"""
        try:
            # Generate hash value of voice
            voice_hash = hashlib.md5(voice.encode()).hexdigest()
