...
class GlobalCacheManager:
    ...
    def set(
        ...
        # Use configured TTL or incoming TTL
        ...
        with self._locks[cache_name]:
            ...
            if config.strategy in [CacheStrategy.LRU, CacheStrategy.TTL_LRU]:
                # LRU policy: move to end if already exists
                if key in cache:
                    del cache[key]
                cache[key] = entry
                ...
                # Check size limits
                if config.max_size and len(cache) > config.max_size:
                    # Simple strategy: remove an entry at random
    ...
    def invalidate_pattern(
        ...
        """Invalidate cache entries by mode"""
        cache_name = self._get_cache_name(cache_type, namespace)
    ...
    def _maybe_cleanup(self, cache_name: str):
        ...
        if now - self._last_cleanup > config.cleanup_interval:
            ...
            if deleted > 0:
                ...
                self.logger.debug(f"Clean cache {cache_name}: delete {deleted} expired entries")
