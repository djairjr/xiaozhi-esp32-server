...
class VoiceprintProvider:
    ...
    def __init__(self, config: dict):
        ...
        # Parse API addresses and keys
        self.api_url = None
        self.api_key = None
        self.speaker_ids = []
        
        if not self.original_url:
            logger.bind(tag=TAG).warning("The voiceprint recognition URL is not configured and voiceprint recognition will be disabled.")
            self.enabled = False
        else:
            # Parse URL and key
            ...
            # Extract keys from query parameters
            ...
            if not self.api_key:
                ...
                # Construct the identify interface address
                ...
                # Check if there is a valid speaker configuration
                if not self.speaker_ids:
                    logger.bind(tag=TAG).warning("No valid speaker is configured, voiceprint recognition will be disabled")
                    self.enabled = False
                else:
                    # Perform a health check to verify that the server is available
                    if self._check_server_health():
                        self.enabled = True
                        logger.bind(tag=TAG).info(f"Voiceprint recognition is enabled: API={self.api_url}, speakers={len(self.speaker_ids)}, similarity threshold={self.similarity_threshold}")
                    else:
                        self.enabled = False
                        logger.bind(tag=TAG).warning(f"The voiceprint recognition server is unavailable and voiceprint recognition is disabled: {self.api_url}")
    ...
    def _check_server_health(self) -> bool:
        """Check the health status of the voiceprint recognition server"""
        ...
        if cached_result is not None:
            logger.bind(tag=TAG).debug(f"Use cached health: {cached_result}")
            return cached_result
        
        # Cache expires or does not exist
        logger.bind(tag=TAG).info("Perform voiceprint server health check")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if result.get("status") == "healthy":
                    ...
                    logger.bind(tag=TAG).warning(f"Voiceprint recognition server status is abnormal: {result}")
                    is_healthy = False
            else:
        ...
        # Caching results using the global cache manager
        cache_manager.set(CacheType.VOICEPRINT_HEALTH, cache_key, is_healthy)
        logger.bind(tag=TAG).info(f"Health check results cached: {is_healthy}")
    ...
    async def identify_speaker(self, audio_data: bytes, session_id: str) -> Optional[str]:
        ...
        if not self.enabled or not self.api_url or not self.api_key:
            logger.bind(tag=TAG).debug("The voiceprint recognition function is disabled or not configured, and the recognition is skipped.")
            return None
                ...
                async with session.post(self.api_url, headers=headers, data=data) as response:
                    ...
                    if response.status == 200:
                        ...
                        if score < self.similarity_threshold:
                            logger.bind(tag=TAG).warning(f"Voiceprint recognition similarity {score:.3f} is lower than the threshold {self.similarity_threshold}")
