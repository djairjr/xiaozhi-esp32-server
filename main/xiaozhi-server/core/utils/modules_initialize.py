...
def initialize_modules(
    ...
    if init_tts:
        ...
        logger.bind(tag=TAG).info(f"Initialization component: tts successful {select_tts_module}")
    ...
    if init_memory:
        ...
        logger.bind(tag=TAG).info(f"Initialization component: memory successful {select_memory_module}")
    ...
    if init_vad:
        ...
        logger.bind(tag=TAG).info(f"Initialization component: vad successful {select_vad_module}")

    # Initialize the ASR module
    if init_asr:
        select_asr_module = config["selected_module"]["ASR"]
        modules["asr"] = initialize_asr(config)
        logger.bind(tag=TAG).info(f"Initialization component: asr successful {select_asr_module}")
    return modules
...
def initialize_voiceprint(asr_instance, config):
    ...
    try:
        ...
        logger.bind(tag=TAG).info("The voiceprint recognition function of the ASR module has been dynamically enabled")
