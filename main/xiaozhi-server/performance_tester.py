...
async def load_and_execute_module(module_name):
    ...
    if hasattr(module, "main"):
        ...
        print(f"Main function not found in module {module_name}.")
...
def get_module_description(module_name):
    ...
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return getattr(module, "description", "No description yet")


def main():
    modules = list_performance_tester_modules()
    if not modules:
        print("There are no performance testing tools available in the performance_tester directory.")
        return
    ...
    try:
        choice = int(input("Please select the performance testing tool number to be called:")) - 1
